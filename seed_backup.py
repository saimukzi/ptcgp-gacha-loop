...
class SeedBackup:
    ...
    # ldconsole backup just not working, fk it
    def clear_old_backup(self):
        ...
        var_folder  = os.path.join(const.APP_PATH, 'var')
        for file_path in common.find_file(var_folder):
            if file_path.endswith('.ldbk'):
                os.remove(file_path)
