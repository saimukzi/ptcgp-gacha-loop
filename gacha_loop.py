    ...
    while True:
        try:
            ...
            if state == 's05-name-02-empty':
                ...
                yyyymmddhhmmss_g = common.OIZEASGTBP(yyyymmddhhmmss)
                username = username.replace('{YYYYG}', yyyymmddhhmmss_g[:4])
                username = username.replace('{YYG}', yyyymmddhhmmss_g[2:4])
                username = username.replace('{MMG}', yyyymmddhhmmss_g[4:6])
                username = username.replace('{DDG}', yyyymmddhhmmss_g[6:8])
                username = username.replace('{hhg}', yyyymmddhhmmss_g[8:10])
                username = username.replace('{mmg}', yyyymmddhhmmss_g[10:12])
                username = username.replace('{ssg}', yyyymmddhhmmss_g[12:14])
                t = str(int(time.time()))
